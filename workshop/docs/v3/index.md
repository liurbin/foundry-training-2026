...
3. **评测**：让 codex CLI 帮你写 4 条 case（happy / edge / 两条对抗），跑出明确 pass/fail
...
上线边界单独放在 [Enterprise Readiness：12 个上线边界](enterprise-readiness.md)。真实项目迁移使用 [AI Solution Builder Compass](ai-solution-readiness-blueprint.md)：先把 builder 必须 top-of-mind 的 workflow、动作边界、eval、安全、平台漂移问题问清楚，再做 Foundry-specific 文档取证和实现路径。
...
- 用 [AI Solution Builder Compass](ai-solution-readiness-blueprint.md) 评估你的真实场景，先问清 workflow / action / eval / risk，再做 Foundry-specific mapping
- 把 mock 业务数据替换成真接口
