# InstructorHandbook v2：决策模块交付物

> Bilingual mirror. Original source: `docs/01-instructor-handbook-v2.md`.
...
- Chinese canonical title: 讲师手册 v2：决策模块交付物
- English navigation title: InstructorHandbook v2：决策模块交付物
- Scope: this page is part of the full bilingual Foundry training site.

## Chinese Source with Bilingual Headings

# InstructorHandbook v2：决策模块交付物 / 讲师手册 v2：决策模块交付物
...
````markdown
# Agent Service vs Agent Framework SDK Decision Card / Agent Service vs Agent Framework SDK 选型决策卡
...
## 决策规则（Instructor给的启发式） / 决策规则（讲师给的启发式）
...
### 部署路径分层（Instructor必须开篇讲清） / 部署路径分层（讲师必须开篇讲清）
...
````markdown
# Ask AI to Deploy the D3 Single Agent to Hosted Agents, then Add Retry, Cache, and Cost Estimation / 让 AI 帮我把 D3 的单 agent 部署到 Hosted Agents（主路径），并补 429 重试 + 缓存 + 成本估算；ACA 仅作对照

## 部署路径分层（开课前Instructor必须先讲清） / 部署路径分层（开课前讲师必须先讲清）
...
## D6a — Agent Framework SDK 的Boundary / D6a — Agent Framework SDK 的边界
...
## D6b — A2A + MCP 的Boundary / D6b — A2A + MCP 的边界
...
### InstructorPreparation（**这条 plan 已经依赖，必须 Day-7 完成**） / 讲师准备（**这条 plan 已经依赖，必须 Day-7 完成**）
...
## D9 — Production checklist / D9 — 生产化 checklist
...
## D10 — Foundry 能力Boundary表 / D10 — Foundry 能力边界表
...
````markdown
# Ask AI to Map My Project to the Foundry Capability Boundary Table / 让 AI 帮我对照 Foundry 能力边界表，找出我项目命中的边界

## Inputs / 输入
- plan 第五·五能力地图 + 本节边界表（讲师 Day3 发）
- 我项目 D2 / D6a 决策卡产物

## Boundary表（Instructor当天发实物，Learner对照勾选） / 边界表（讲师当天发实物，学员对照勾选）
...
> **预填 URL 的限制**（讲师必看）：以上 URL 来源于 agent 在 2026/05 调 WebFetch 抓取核对；agent 没有 Foundry 订阅 / portal 权限，所以 portal 截图、fork 实测、订阅级 region 可用性这三类**仍是 Day-7 实证项**。学员看到只有 URL 没有 portal 截图的行，应当反馈讲师 Day-7 补。
...
````markdown
# Team Spec Template Library Setup Guide (Instructor Demo) / 团队 spec 模板库搭建指南（讲师演示用）
...
## 2 个示例 spec 大纲（Learner誊抄/裁剪） / 2 个示例 spec 大纲（学员誊抄/裁剪）
